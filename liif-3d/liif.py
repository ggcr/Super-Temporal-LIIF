...
class LIIF(nn.Module):
    ...
    def query_rgb(self, coord, cell=None):
        ...
        if self.feat_unfold:
            res = torch.empty((feat.shape[0], feat.shape[1] * 9, feat.shape[2], feat.shape[3], feat.shape[4]), dtype=feat.dtype)
            for i in range(feat.shape[2]):
                res[:,:,i,:,:] = F.unfold(feat[:,:,i,:,:], 3, padding=1).view(
                    feat.shape[0], feat.shape[1] * 9, feat.shape[3], feat.shape[4])
            feat = res.cuda()
        ...
        feat_coord = feat_coord.permute(-1,0,1,2)
        feat_coord = feat_coord.unsqueeze(0).expand(feat.shape[0], 3, *feat.shape[-3:])
        ...
        if self.local_ensemble:
            ...
            t = areas[2]; areas[2] = areas[-3]; areas[-3] = t
